...
int input_read_parameters(
  ...
  if ((flag1 == _TRUE_) && (N_ncdm > 0)){
    ...
    /* Allocate extra flags for SIWDM */

    class_alloc(pba->mass_set_by_DF,sizeof(int)*pba->N_ncdm,pba->error_message);
    class_alloc(pba->Omega_set_by_DF,sizeof(int)*pba->N_ncdm,pba->error_message);
    class_alloc(pba->is_NR_SI_decoupled,sizeof(int)*pba->N_ncdm,pba->error_message);
    ...
    for (n=0; n < N_ncdm; n++){
      ...
      /* Extra flags for SIWDM calculations, if we need to re-initialize the species*/
      pba->mass_set_by_DF[n] = _FALSE_;
      pba->Omega_set_by_DF[n] = _FALSE_;
    ...
    fileentries=0;
    if ( (N_ncdm>0) && (pba->N_si_ncdm>0) ){
      for(n=0,fileentries=0; n<N_ncdm; n++){
        if (pba->ncdm_si_type[n] == 1) fileentries++;
      }

      if (fileentries > 0) {

        /* Okay, read filenames.. */
        class_call(parser_read_list_of_strings(pfc,"ncdm_self_interaction_tau_filenames",
                                               &entries_read,&(pba->ncdm_si_tau_files),&flag2,errmsg),
                   errmsg,
                   errmsg);
        class_test(flag2 == _FALSE_,errmsg,
                   "Input ncdm_self_interaction_type is set to Tau_rel(q), but no filenames found!");
        class_test(entries_read != fileentries,errmsg,
                   "Number of filenames found, %d, does not match number of Tau_rel(q) values in ncdm_self_interaction_filenames, %d",
                   entries_read,fileentries);
      }

      class_call(background_si_ncdm_init(ppr, pba),
                                          pba->error_message,
                                          errmsg);
  ...
  */
  ...
  /*SIWDM*/
  //A few new flags
  //class_read_int("ncdm_si_tca_approximation",ppr->ncdm_si_tca_approximation);
  class_read_double("ncdm_si_tca_trigger",ppr->ncdm_si_tca_trigger);
  class_read_double("ncdm_si_initial_tau_trigger",ppr->ncdm_si_initial_tau_trigger);

  /*New ncdm parameters*/
  class_read_int("ncdm_ic_estimation_method", ppr->ncdm_ic_estimation_method);
  class_read_int("ncdm_lmax_method", ppr->ncdm_lmax_method);

  //printf("Passed extra block for ncdm params\n");
...
int input_default_params(
  ...
  /* SI parameters */
  /*Non Relativistic Self Interaction Decoupling Method - Default: ignored*/
  pba->ncdm_NR_SI_decoupling_method = 0;
...
int input_default_precision ( struct precision * ppr ) {
  ...
  /*SIWDM*/
  //ppr->ncdm_si_tca_approximation = ncdmtca_mb;
  ppr->ncdm_si_tca_trigger = 30.;
  ppr->ncdm_si_initial_tau_trigger = 30. * ppr->ncdm_si_tca_trigger;

  /*New ncdm parameters*/
  ppr->ncdm_ic_estimation_method = 0;
  ppr->ncdm_lmax_method = 0;
