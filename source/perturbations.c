 ...
 *
 * The changes made to this module are also minor: in order to account for
 * SIWDM, a few extra flags were added in the ncdm functions themselves
 * to account for the collision term in the TCA.
 *
 * In fact, most of the additions have to do with the tight-coupling approximation.
 * Indeed, we have added this approximation to the ncdm module in case there is 
 * SIWDM present in the same way other approximations are handled,
 * and added the code to switch between the different approximation regimes.
 *
 * We added also a few methods for closing the hierarchies at high l in the TCA,
 * which include tridiagonal matrix solving algorithms. Also included (but not used)
 * are different approaches to close the Boltzmann hierarchy at l_max
 *
 * We also added a bunch of console output for debugging purposes.
 */
...
int perturb_init(
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    class_test ((ppr->ncdm_ic_estimation_method < zeros_method) ||
                (ppr->ncdm_ic_estimation_method > tridiag_method),
                ppt->error_message,
                "your ncdm_ic_estimation_method is set to %d, out of range defined in perturbations.h",ppr->ncdm_ic_estimation_method);

    class_test ((ppr->ncdm_lmax_method < mb_method) ||
                (ppr->ncdm_lmax_method > y_method),
                ppt->error_message,
                "your ncdm_lmax_method is set to %d, out of range defined in perturbations.h",ppr->ncdm_lmax_method);

    class_test ((ppr->ncdm_lmax_method == y_method) &&
                (pba->has_curvature==_TRUE_),
                ppt->error_message,
                "You have set ncdm_lmax_method to y_method, which is incompatible with nonzero curvature.");
...
#pragma omp for schedule (dynamic)
        ...
        for (index_k = ppt->k_size[index_md]-1; index_k >=0; index_k--) {
          ...
          if ((ppt->perturbations_verbose > 1) && (abort == _FALSE_)) {
...
int perturb_workspace_init(
  ...
  if (_scalars_) {
    ...
    /*SIWDM - Define the indexes for the different approximation regimes*/

    if (ppt->perturbations_verbose>4) printf("In perturb_workspace_init: pba->has_si_ncdm = %d \n", pba->has_si_ncdm);
    class_define_index(ppw->index_ap_ncdmtca_ncdm1, pba->has_si_ncdm, index_ap, pba->N_ncdm);

    if (ppt->perturbations_verbose>4) {
      if(pba->has_si_ncdm== _TRUE_ ){
       printf("In perturb_workspace_init: index_ap_ncdmtca_ncdm1 defined as %d \n", ppw->index_ap_ncdmtca_ncdm1);
     }
    }
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (pba->has_si_ncdm == _TRUE_){
        ...
        for (n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++){
          ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] = (int)ncdmtca_on;
        }
      }
    }
  }
...
int perturb_solve(
  ...
  //if(ppt->perturbations_verbose>1) printf("pvecback[pba->index_bg_taurel_si_ncdm1] = %e\n", ppw->pvecback[pba->index_bg_taurel_si_ncdm1]);
    ...
    for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++) {
      ...
      if (pba->has_si_ncdm == _TRUE_)  {
        ...
        if (pba->is_early_coupled[n_ncdm] == _TRUE_) {
          ...
          tau_rel = ppw->pvecback[pba->index_bg_taurel_si_ncdm1+n_ncdm] / ppw->pvecback[pba->index_bg_a] ;

          if(ppt->perturbations_verbose>3){
            printf("Testing si_ncdm_tca approxiamtion at initial testing time pba->tau_table[0] ...\n");
            printf("At this time: \n");

            printf("tau_init= %e ", tau_lower);
            printf("tau_rel=%e ", tau_rel);
            printf("tau_k=%e ", 1./k);
            printf("tau_h=%e ", 1./(ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_a]));

            printf("is_early_coupled set to %d\n", pba->is_early_coupled[n_ncdm] );
          }
        }
      }
    }
  ...
  // We assume that if the species is sufficiently relativistic then it is early enough, 
  // here we just test it through console output
  if(pba->has_si_ncdm == _TRUE_){
    if(ppt->perturbations_verbose>2){
      for(n_ncdm=0;n_ncdm<pba->N_ncdm;n_ncdm++){
        tau_rel = ppw->pvecback[pba->index_bg_taurel_si_ncdm1+n_ncdm] / ppw->pvecback[pba->index_bg_a] ;
        printf("Found minimum tau value to be %e for mode k=%e. For this value, tau_rel=%e, tau_k=%e and tau_h=%e\n",
              tau,
              k,
              tau_rel,
              1./k,
              1./(ppw->pvecback[pba->index_bg_H]*ppw->pvecback[pba->index_bg_a]));
      }
    }
  }
  ...
  if(ppt->perturbations_verbose>6) printf("In perturb_solve: attempting to free memory! \n");
  ...
  if(ppt->perturbations_verbose>6) printf("In perturb_solve: memory freed! \n");
...
int perturb_find_approximation_number(
  ...
  *interval_number=1;
  ...
  for (index_ap=0; index_ap<ppw->ap_size; index_ap++) {
    ...
    if (pba->has_si_ncdm == _TRUE_){
      if(index_ap == ppw->index_ap_ncdmtca_ncdm1){
        if (ppt->perturbations_verbose>5){
          printf("perturb_find_approximation_number called at k= %e. For initial time tau = %e, index_ap_ncdmtca_ncdm1 approximation is set to: %d\n",
            k,
            tau_ini,
            ppw->approx[index_ap]);
        }
    ...
    if (pba->has_si_ncdm == _TRUE_){
      if(index_ap == ppw->index_ap_ncdmtca_ncdm1){
        if (ppt->perturbations_verbose>5){
          printf("perturb_find_approximation_number called at k=%e. For end time tau = %e, index_ap_ncdmtca_ncdm1 approximation is set to: %d\n",
            k,
            tau_end,
            ppw->approx[index_ap]);
        }
...
int perturb_find_approximation_switches(
  ...
  if (pba->has_si_ncdm == _TRUE_ ){
    if (ppt->perturbations_verbose>2){
      printf("perturb_find_approximation_switches called. Number of intervals set to: %d\n at mode k=%e", interval_number, k );
      printf("Number of intervals with respect to ncdm_tca for species 0 at mode k=%e: %d \n",
      k,
      interval_number_of[ppw->index_ap_ncdmtca_ncdm1] );
    }
  }
  ...
  if (pba->has_si_ncdm == _TRUE_){
    if(ppt->perturbations_verbose>2){
      for (n_ncdm =0; n_ncdm<pba->N_ncdm; n_ncdm++){
        printf("At mode k=%e, at aproximation interval %d, ncdm_tca for species %d is set to %d \n", 
                k,
                0,
                n_ncdm,
                interval_approx[0][ppw->index_ap_ncdmtca_ncdm1+n_ncdm]);
      }
    }
  }
  ...
  /** - if there are switches, consider approximations one after each
    ...
    for (index_ap=0; index_ap<ppw->ap_size; index_ap++) {
      ...
      if (interval_number_of[index_ap] > 1) {
        ...
        if(ppt->perturbations_verbose>4){
          printf("Switch for mode k=%e with approximation number %d found at tau=%e \n",k , index_ap, mid );
        }
      }
    }
    ...
    if (ppt->perturbations_verbose>4){
      printf("Interval limits for k=%e found to be:\n", k);
      for(index_switch=0; index_switch<=interval_number; index_switch++){
        printf("%e \t ", interval_limit[index_switch]);
      }
      printf("\n");
    }
    ...
    for (index_switch=1; index_switch<interval_number; index_switch++) {
      ...
      for (index_ap=0; index_ap<ppw->ap_size; index_ap++) {
        ...
        if (pba->has_si_ncdm == _TRUE_ ){
          if (ppt->perturbations_verbose>2) {
            for (n_ncdm = 0; n_ncdm<pba->N_ncdm; n_ncdm++){
              if (index_ap == ppw->index_ap_ncdmtca_ncdm1 + n_ncdm){
                printf("At mode k=%e, at aproximation interval %d, ncdm_tca for species %d is set to %d. \n", 
                        k,
                        index_switch,
                        n_ncdm,
                        interval_approx[index_switch][ppw->index_ap_ncdmtca_ncdm1+n_ncdm]);
              }
            }
          }
        }
      ...
      for (index_ap=0; index_ap<ppw->ap_size; index_ap++) {
        if (interval_approx[index_switch][index_ap] != interval_approx[index_switch-1][index_ap]){
          ...
          /*printf("num_switching at given time increased for mode %e!. It has detected that approximation %d has changed for interval %d\n",
          k,
          index_ap,
          index_switch );*/
        }
      ...
      /*SIWDM*/
      if (pba->has_si_ncdm == _TRUE_ ){
        if( (num_switching_at_given_time != 1) ){
          for(n_ncdm=0;n_ncdm<pba->N_ncdm;n_ncdm++){
            class_test(  ( ( (interval_approx[index_switch-1][ppw->index_ap_ncdmtca_ncdm1+n_ncdm]==(int)ncdmtca_on) &&
                              (interval_approx[index_switch][ppw->index_ap_ncdmtca_ncdm1+n_ncdm]==(int)ncdmtca_off) ) && 
                            ( (interval_approx[index_switch-1][ppw->index_ap_ncdmfa]==(int)ncdmfa_off) &&
                              (interval_approx[index_switch][ppw->index_ap_ncdmfa]==(int)ncdmfa_on) ) ) ,
                          ppt->error_message ,
                          "for k=%e, at tau=%g you switch both non cold dm species %d fluid approximation and tight coupling at the same time, this cannot be handled. This may be caused by the TCA approximation switching on and off during evolution, which is not yet implemented. ",
                          k,
      ...
      if (ppt->perturbations_verbose>2) {
        ...
        if (_scalars_) {
          ...
          if (pba->has_ncdm == _TRUE_) {
            ...
            if (pba->has_si_ncdm == _TRUE_){
              for (n_ncdm =0; n_ncdm<pba->N_ncdm; n_ncdm++){
                if ((interval_approx[index_switch-1][ppw->index_ap_ncdmtca_ncdm1+n_ncdm]==(int)ncdmtca_on) &&
                    (interval_approx[index_switch][ppw->index_ap_ncdmtca_ncdm1+n_ncdm]==(int)ncdmtca_off)) {
                  fprintf(stdout,"Mode k=%e: will switch off ncdm self-interaction TCA approximation for species %d at tau=%e\n",k,n_ncdm,interval_limit[index_switch]);
                }
              }
            }
            //printf("In perturb_find_approximation_switches, passed logic switch\n");
          }
        }
 ...
 * which is freed.
 *
 * MODIFIED FOR THE SIWDM MODULE
 * 
 * This function carries a few modifications in this module, mainly basic 
 * things, such as initializing a few relevant variables, checking
 * for consistency in the integration and implementing the switches
 * for the new approxiamtion schemes.
 * 
 * @param ppr        Input: pointer to precision structure
 * @param pba        Input: pointer to background structure
 * @param pth        Input: pointer to the thermodynamics structure
...
int perturb_vector_init(
  ...
  int index_pt;
  int l;
  int n_ncdm,n2_ncdm,index_q,ncdm_l_size;
  double rho_plus_p_ncdm,q,q2,epsilon,qk_div_epsilon,a,factor;
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for(n_ncdm = 0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        // The cases are separate if has_si_ncdm is true or not: if there is no si_ncdm then the approximation indices are not defined!
        if (pba->has_si_ncdm == _TRUE_){

          if( (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_off) 
            && (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int) ncdmtca_off) ) {
            //Case of no approximations
            /* reject inconsistent values of the number of mutipoles in ultra relativistic neutrino hierarchy */
          ...
          else if ( (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) 
            && (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int) ncdmtca_off) ){
            ...
            // In the ncdm tight coupling approximation, hierarchy is cut at lmax=1 (Psi_{l>=2} can be calculated from Psi_1 in this limit)
  ...
  if (pa_old == NULL) {
    ...
    if (_scalars_) {
      ...
      if (pba->has_ncdm == _TRUE_) {
        ...
        if (pba->has_si_ncdm == _TRUE_) {
          ...
          for(n_ncdm = 0; n_ncdm<pba->N_ncdm; n_ncdm++){
            ...
            if(ppt->perturbations_verbose>3){
              printf("New vector instance initialized: n_ncdm= %d, ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm]= %d, is_early_coupled=%d \n",
                    n_ncdm, 
                    ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm],
                    pba->is_early_coupled[n_ncdm]);
            }
      ...
      if(ppt->perturbations_verbose>4) printf("Scalar initial conditions consistent with approximations! \n");
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      /* -- case of switching on ncdm fluid
         approximation. Provide correct initial conditions to new set
         of variables */
      /* NEW FOR SIWDM MODULE: this switch changes in case it comes from the new TCA*/

      if (pba->has_ncdm == _TRUE_) {

        for(n_ncdm = 0; n_ncdm < ppv->N_ncdm; n_ncdm++){

          //Cases need to be separates if has_si_ncdm is on, as approximation indices are not defined otherwise
          if (pba->has_si_ncdm == _TRUE_){

            /* In the case of switching fluid approx for SI-WDM:
             Only do all of this if SI tight coupling is off. If it is, just copy the variables */

            if ((pa_old[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_off) && (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_off)){

              if ((pa_old[ppw->index_ap_ncdmfa] == (int)ncdmfa_off) && (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on)) {

                //Case: si_ncdm tight coupling is off: perform the switch to fluid approximation 

                if (ppt->perturbations_verbose>2)
                  fprintf(stdout,"Mode k=%e: switch on ncdm fluid approximation at tau=%e. ncdm_TCA is OFF\n",k,tau);
                ...
                T_over_m = ( pba->T_cmb * pba->T_ncdm[n_ncdm] / a) * (_k_B_) /
                ...
                for(index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++){
                  ...
                  /* NR SIWDM decoupling: get the additional integration factor*/
            ...
            else if ((pa_old[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_on) && (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_on)){

              if ((pa_old[ppw->index_ap_ncdmfa] == (int)ncdmfa_off) && (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on)) {

                //Case: si_ncdm tight coupling is on: TCA overrides fluid approximation and the variables are just copied
              ...
              class_test((pa_old[ppw->index_ap_ncdmfa] == (int)ncdmfa_off) && (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on),
                ppt->error_message,
                "Trying to switch both ncdm_tca and ncdm_fa at the same time. Something went wrong!");
            }
          }
          //If no SIWDM, don't evaluate the ncdmtca approximation indices, as they are not defined
          else{
      ...
      /* -- case of switching off ncdm tight coupling
         approximation. Provide correct initial conditions to new set
         of variables */

      if ( (pba->has_ncdm == _TRUE_) && (pba->has_si_ncdm == _TRUE_) ) {

        for (n_ncdm = 0; n_ncdm<pba->N_ncdm; n_ncdm++){

          if ((pa_old[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_on) && (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_off)) {
            ...
            /* Here, we will have to differentiate between individual species, that may switch off TCA at different times...*/
            index_pt = 0;
            for(n2_ncdm = 0; n2_ncdm < pba->N_ncdm; n2_ncdm++){
              //If the species is not n_ncdm, copy
              if(n2_ncdm!=n_ncdm){
                for(index_q=0; index_q < ppv->q_size_ncdm[n2_ncdm]; index_q++){
                  for(l=0; l<=ppv->l_max_ncdm[n2_ncdm]; l++){
                    // This is correct even when ncdmfa == off, since ppv->l_max_ncdm and ppv->q_size_ncdm is updated.
                    ppv->y[ppv->index_pt_psi0_ncdm1+index_pt] =
                      ppw->pv->y[ppw->pv->index_pt_psi0_ncdm1+index_pt];
                    index_pt++;
                  }
              ...
              //If the species is n_ncdm, replace the appropriate values...
              else{
                /* First case: if the switch is from ncdm tight coupling to no approximations... */
                if (ppw->approx[ppw->index_ap_ncdmfa] == (int) ncdmfa_off){

                  //fprintf(stderr, "Changing to ncdmtca_off in ncdmfa_off: k=%e \t tau= %e\n", k, tau);

                  //We copy the values of Psi0, Psi1 and initialize the values for Psi_(l>=2) to their equilibrium values
                  ...
                  for(index_q=0; index_q < ppv->q_size_ncdm[n_ncdm]; index_q++){
                    ...
                    for(l=0; l<2; l++){
                      ppv->y[ppv->index_pt_psi0_ncdm1+index_pt] =
                        ppw->pv->y[ ppw->pv->index_pt_psi0_ncdm1 + index_q*(ppw->pv->l_max_ncdm[n_ncdm] + 1) + l];
                      index_pt++;
                      //fprintf(stderr, " %d ", index_pt);
                    }

                    //In this block we perform tridiagonal matrix solving (from get_ncdmtca_psi_high_l) to get Psi_(l>=2)
                    a = ppw->pvecback[pba->index_bg_a];
                    q = pba->q_ncdm[n_ncdm][index_q];
                    q2 = q*q;
                    epsilon = sqrt(q2+a*a*pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm]);
                    ...
                    //  fprintf(stderr, "Detected qk_div_epsilon above threshold: tau_rel*qk_div_epsilon=%e tolerance=%e\n", tau_rel*qk_div_epsilon, ppr->tol_ncdm_tca_tauqk_div_epsilon );

                    class_call(get_ncdmtca_psi_high_l(pba , 
                                                      ppt , 
                                                      ppw ,
                                                      ppw->pv->y[ppw->pv->index_pt_psi0_ncdm1 + index_q*(ppw->pv->l_max_ncdm[n_ncdm] + 1) + 1] ,
                                                      tau,
                                                      k,
                                                      n_ncdm,
                                                      index_q,
                    ...
                    for(l=2; l<=ppv->l_max_ncdm[n_ncdm]; l++){
                      /* This is correct even when ncdmfa == off, since ppv->l_max_ncdm and
                          ppv->q_size_ncdm is updated.*/
                      ppv->y[ppv->index_pt_psi0_ncdm1+index_pt] =
                        ppw->tridiag_d[l-2] ;
                ...
                /*Otherwise, if the switch is to a fluid approximation condition: We calculate delta_ncdm, theta_ncdm 
                and initialize sigma_ncdm: a term accounting for SI is added in the derivative afterwards.
                Still, it is not recommended to use FA in the case of SI-WDM, as I'm not sure of its full applicability */
                  ...
                  /* NR SIWDM decoupling calculations */
                  ...
                  for(index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++){
                    ...
                    //ppv->y[ppv->index_pt_psi0_ncdm1+ncdm_l_size*n_ncdm+2] = 0.0;
    ...
    if (ppt->perturbations_verbose>6) printf("In perturb_vector_init: Attempting to call perturb_vector_free!\n");

    class_call(perturb_vector_free(ppw->pv),
               ppt->error_message,
               ppt->error_message);

    if (ppt->perturbations_verbose>6) printf("In perturb_vector_init: Called perturb_vector_free!\n");
 ...
 * 
 * SLIGHT MODIFICATION FOR SIWDM MODULE
 * For the case of SIWDM, the species is generally assumed to start in TCA,
 * so we need to modify lightly the initial conditions in this case to, 
 * for example, not include shear stress
 *
 * @param ppr        Input: pointer to precision structure
 * @param pba        Input: pointer to background structure
 * @param ppt        Input: pointer to the perturbation structure
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  double dlnf0_dlnq_ncdm, T_over_m;
  ...
  if (_scalars_) {
    ...
    */
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (ppt->perturbations_verbose>5) printf("Setting up ncdm initial conditions... \n");
      ...
      for (n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        if (pba->has_si_ncdm == _TRUE_){
          ...
          if ( (pba->is_early_coupled[n_ncdm] == _TRUE_) && (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_on ) ){

            // Here, we handle the case of tight coupling approximation. In this case, we only track 0 and 1 moments 
            ...
            for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++) {
              ...
              ppw->pv->y[idx] = -0.25 * delta_ur * dlnf0_dlnq_ncdm;

              //ppw->pv->y[idx+1] =  -epsilon/3./q/k*theta_ur * pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
              ppw->pv->y[idx+1] =  -epsilon/3./q/k*theta_ur * dlnf0_dlnq_ncdm;              

              //ppw->pv->y[idx+2] = 0.0; //-0.5 * shear_ur * pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
            ...
            if(ppt->perturbations_verbose>3) printf("Scalar initial conditions for mode k=%e and ncdm species %d defined with Psi_l>2 = 0, as pba->is_early_coupled[n_ncdm]= %d\n", k, n_ncdm, pba->is_early_coupled[n_ncdm] );
          }
          else{

            //Otherwise, define them regularly from their ur counterparts
          ...
          //Here, the case of no si_ncdm: Initial conditions set as before for ncdm

          if (ppt->perturbations_verbose>6) printf("In perturb_initial_conditions: Detected has_si_ncdm as _FALSE_!\n");
      ...
      if (ppt->perturbations_verbose>6) printf("Finished setting up initial conditions for ncdm!\n");
 ...
 * 
 * Added conditions for the SIWDM TCA and relativistic to nonrelativistic
 * transition in the case of NR self decoupling
 *
 * @param ppr        Input: pointer to precision structure
 * @param pba        Input: pointer to background structure
 * @param pth        Input: pointer to thermodynamics structure
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      if (pba->has_si_ncdm == _TRUE_){
        for(n_ncdm = 0; n_ncdm<pba->N_ncdm; n_ncdm++){
          ...
          /*Here, we define the ncdm tight coupling approximation. Is should be set if tau_rel is both smaller than
          tau_h and tau_k*mean_epsilon. If tau_rel is either inf or a variation of NAN, then we assume it underflowed,
          so it should be very small (we interpolate it from a log-log table, any negative values cause this)*/
          /* Why the a? We expect tables in physical values, need to be converted to comoving! */

          a = ppw->pvecback[pba->index_bg_a];
          tau_rel = ppw->pvecback[pba->index_bg_taurel_si_ncdm1+n_ncdm] / a ;
          mean_epsilon = sqrt( 1 + a*a*pba->M_ncdm[n_ncdm]*pba->M_ncdm[n_ncdm] );

          if ( ( tau_rel / ( tau_k * mean_epsilon )  < 1./ ppr->ncdm_si_tca_trigger ) &&
          ( tau_rel / tau_h < 1./ ppr->ncdm_si_tca_trigger ) ) {

            ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] = (int) ncdmtca_on;

            if (ppt->perturbations_verbose>6){
              printf("Calculated ncdm_tca approx to be ON for species %d at time tau=%e, temperature T=%e for k=%e. Tau_rel=%e, Tau_k= %e, Tau_h= %e \n",
                    n_ncdm, 
                    tau, 
                    pba->T_cmb * pba->T_ncdm[n_ncdm] * (1./ppw->pvecback[pba->index_bg_a]*pba->a_today),
                    k,
                    tau_rel,
                    tau_k,
                    tau_h );
          ...
          else if ( isnormal(tau_rel) ) {

            ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] = (int) ncdmtca_off;

            if (ppt->perturbations_verbose>6) {
              printf("Calculated ncdm_tca approx to be OFF for species %d at time tau=%e, temperature T=%e for k=%e. Tau_rel=%e, Tau_k= %e, Tau_h= %e \n",
                    n_ncdm, 
                    tau,
                    pba->T_cmb * pba->T_ncdm[n_ncdm] * (1./ppw->pvecback[pba->index_bg_a]*pba->a_today),
                    k,
            ...
            if (ppt->perturbations_verbose>1){
              printf("WARNING! relaxation time underflow. Calculated ncdm_tca approx to be ON for species %d at time tau=%e, temperature T=%e for k=%e. Tau_rel=%e, Tau_k= %e, Tau_h= %e \n",
                    n_ncdm, 
                    tau, 
                    pba->T_cmb * pba->T_ncdm[n_ncdm] * (1./ppw->pvecback[pba->index_bg_a]*pba->a_today),
                    k,
          ...
          //ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] = (int) ncdmtca_on;
          if(ppt->perturbations_verbose>6){
            printf("Executed perturb_approximations: Self Interaction Tight Coupling approximation set to %d\n"
              , ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] );
          }
        }
      }
 ...
 * MODIFIED FOR SIWDM MODULE
 * 
 * If we are not in the TCA, we need to include the relaxation time 
 * in the list of relevant timescales.
 *
 * @param tau                      Input: conformal time
...
int perturb_timescale(
  ...
  /* (c) time scale of recombination, \f$ \tau_{\gamma} = 1/\kappa' \f$ */
  double tau_c;
  /* (d) time scale of self interactions, conformal relaxation time*/
  double tau_rel;
  ...
  if ((ppt->has_scalars == _TRUE_) && (pppaw->index_md == ppt->index_md_scalars)) {
    ...
    *timescale = tau_h;
    ...
    /*SIWDM
     * If I have a SIWDM species that is not on TCA, its relaxation time is also a timescale
     */

    if ( (pba->has_ncdm == _TRUE_) && (pba->has_si_ncdm==_TRUE_) && (pba->sum_early_coupled > 0) ){
      tau_rel=*timescale;
      for (int i=0; i<pba->N_ncdm; i++){
        if (ppw->approx[ppw->index_ap_ncdmtca_ncdm1+i] == (int) ncdmtca_off){
          tau_rel = MIN(pvecback[pba->index_bg_taurel_si_ncdm1 + i] / pvecback[pba->index_bg_a], tau_rel);
        }
      }
      *timescale=MIN(tau_rel,*timescale);
    } 
  }
  ...
  if (_scalars_) {
    ...
    if (pba->has_ncdm == _TRUE_) {
      ...
      for(n_ncdm=0; n_ncdm < pba->N_ncdm; n_ncdm++){
        ...
        //Different cases depending on has_si_ncdm: if there is no si_ncdm then approximation indices for tca are not defined
        if(pba->has_si_ncdm == _TRUE_){
          ...
          if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on 
            ...
            //printf("Adding %e to to rho_delta_ncdm, %e to rho_plus_p_theta_ncdm and %e to rho_plus_p_shear_ncdm \n", rho_delta_ncdm, rho_plus_p_theta_ncdm, rho_plus_p_shear_ncdm);
          }
          else if ( ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_on) {
            //We must integrate to find the perturbations but only for l<2 (shear is not tracked!)
            rho_delta_ncdm = 0.0;
            ...
            for(index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q++){
              ...
              /* NR SI decoupling: get the additional integration factor*/
              ...
              rho_delta_ncdm += factor_prime*q2*epsilon*pba->w_ncdm[n_ncdm][index_q]*y[idx];
              rho_plus_p_theta_ncdm += factor_prime*q2*q*pba->w_ncdm[n_ncdm][index_q]*y[idx+1];
              //rho_plus_p_shear_ncdm += q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q]*y[idx+2];
              delta_p_ncdm += factor_prime*q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q]*y[idx];
            ...
            // No approximations, we must integrate to find perturbations:
            rho_delta_ncdm = 0.0;
            rho_plus_p_theta_ncdm = 0.0;
            rho_plus_p_shear_ncdm = 0.0;
            delta_p_ncdm = 0.0;
            ...
            for (index_q=0; index_q < ppw->pv->q_size_ncdm[n_ncdm]; index_q ++) {
              ...
              rho_delta_ncdm += factor_prime*q2*epsilon*pba->w_ncdm[n_ncdm][index_q]*y[idx];
              rho_plus_p_theta_ncdm += factor_prime*q2*q*pba->w_ncdm[n_ncdm][index_q]*y[idx+1];
              rho_plus_p_shear_ncdm += factor_prime*q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q]*y[idx+2];
              delta_p_ncdm += factor_prime*q2*q2/epsilon*pba->w_ncdm[n_ncdm][index_q]*y[idx];

              //printf("Used factor=%e, and y[idx]=%e, y[idx+1]=%e, y[idx+2]=%e \n", factor, y[idx], y[idx+1], y[idx+2]);
            ...
            //For testing a particular mode, uncomment these lines! It will be print to stderr
            //I prefer this method of testing, as there is no need to change the main file at compiling or implement new stuff
            /*
            if ((k<4.7e-3) && (k>4.6e-3)){
              printf(stderr, "At k=%e, a=%e, adding %e to to rho_delta_ncdm, ppw->delta_rho is %e \n",
                k,
                ppw->pvecback[pba->index_bg_a],
                rho_delta_ncdm, 
                ppw->delta_rho);
          ...
          if(ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on ) {
            ...
            /*printf("At k=%e, a=%e, adding %e to to rho_delta_ncdm, ppw->delta_rho is %e \n",
              k,
              ppw->pvecback[pba->index_bg_a],
              rho_delta_ncdm, 
              ppw->delta_rho);*/
...
/**
 * Compute the higher moment Psi's in the si_ncdm_tca approximation
 *
 * This function calculates the higher moment perturbations in the case of a 
 * Tight-Coupling approximation for tSIWDM. In the case of this approximation
 * the higher moment perturbations relax very quickly to equilibrium values, 
 * calculated via this function.
 *
 * This function depends on background parameters at time tau, assumed to be
 * stored in ppw->pvecback, and stores its results in ppw->tridiag_(..)
 *
 * @param pba        Input: pointer to background structure
 * @param ppt        Input: pointer to the perturbation structure
 * @param ppw        Input/Output: pointer to perturb_workspace structure which fields are allocated or filled here
 * @param psi_1      Input: value of Psi_1(q,k,tau)
 * @param tau        Input: conformal time
 * @param k          Input: wavenumber
 * @param n_ncdm     Input: index of ncdm species
 * @param index_q    Input: index of ncdm specie's momentum, running over pba->q_ncdm[n_ncdm]
 * @param l_max      Input: maximum l moment if no approximations would be considered
 */
int get_ncdmtca_psi_high_l( struct background * pba ,
                            struct perturbs * ppt ,
                            struct perturb_workspace * ppw , 
                            double psi_1,
                            double tau,
 ...
 // dlnf0_dlnq = pba->dlnf0_dlnq_ncdm[n_ncdm][index_q];
  ...
  metric_shear = 0.0;
  if (ppt->gauge == synchronous)  metric_shear = k * k * ppw->pvecmetric[ppw->index_mt_alpha];
...
//A simple implementation of tridiagonal matrix solving
//  see https://en.wikibooks.org/wiki/Algorithm_Implementation/Linear_Algebra/Tridiagonal_matrix_algorithm  
int tridiagonal_solve(double* a, double* b, double* c, double* d, int n) {
    /*
 ...
 * MODIFIED FOR SIWDM MODULE
 * 
 * Added collision terms in RTA approximation (mode 1) and relevant derivatives
 * in the TCA regime.
 * 
 * @param tau                      Input: conformal time
 * @param y                        Input: vector of perturbations
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    //TODO: curvature in all ncdm
    if (pba->has_ncdm == _TRUE_) {

      //if (ppt->perturbations_verbose>7) printf("Detected ncdm in perturb_derivs\n");
      ...
      for (n_ncdm=0; n_ncdm<pv->N_ncdm; n_ncdm++) {
        ...
        */
        ...
        //Different cases if has_si_ncdm is on or not, as perturbation indices for ncdm_tca are not defined if there is no si_ncdm
        if (pba->has_si_ncdm == _TRUE_){          
          ...
          if( (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) 
            ...
            if ( (pba->has_si_ncdm == _TRUE_) && (pba->is_early_coupled[n_ncdm] == _TRUE_) ){
              tau_rel = pvecback[pba->index_bg_taurel_si_ncdm1 + n_ncdm] / a ;
            }
            ...
            //in ncdm_tca: calculate the time derivatives of l = 0, 1, estimate the higher moments via get_ncdmtca_psi_high_l
            for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
          ...
          /** - ----> second case: use exact equation (Boltzmann hierarchy on momentum grid) */
            ...
            for (index_q=0; index_q < pv->q_size_ncdm[n_ncdm]; index_q++) {
              ...
              /** We tested here a few different methods to close the (SI)WDM hierarchies,
               * One as was proposed in Ma & Bertschiner '95 and a new experimental method
               * We found similar results for both! */

              if (ppr->ncdm_lmax_method==(int)mb_method){
                ...
                dy[idx+l] = qk_div_epsilon*y[idx+l-1]/(2.*l+1) - (1.+l)*k*cotKgen*y[idx+l];
                //dy[idx+l] = qk_div_epsilon*y[idx+l-1]-(1.+l)*k*cotKgen*y[idx+l];
                //Isn't there an error in this eq? Shouldn't it be y[lmax-1]*qk/e(2l+1) ?


              }
              else if (ppr->ncdm_lmax_method==(int)y_method){

                //dy[idx+l] = qk_div_epsilon*y[idx+l-1]/(2.*l+1) - (1.+l)*qk_div_epsilon/(2.*l+1)*y[idx+l]/((2.*l+1)/tau/qk_div_epsilon + qk_div_epsilon * tau / (2.*l+1));
                dy[idx+l] = -qk_div_epsilon * y[idx+l-1]/(2.*l+1) - (1.+l)*y[idx+l]/log(k*tau);
              ...
              if ( ( pba->ncdm_si_type[n_ncdm] == 1 ) && (pba->is_early_coupled[n_ncdm] == _TRUE_) ){
                ...
                for (l=2; l<pv->l_max_ncdm[n_ncdm]+1 ; l++){
          ...
          /*for(index_q = 0; index_q<pv->q_size_ncdm[n_ncdm]; index_q++){
            for(l = 0; l<=pv->l_max_ncdm[n_ncdm]; l++){
              if((k<6.0e1) && (k>5.01e1)){
                fprintf(stderr, "%e \t %e \t %e \t %d \t %e \t %e \t %e\n",
                k,
                tau,
                pba->q_ncdm[n_ncdm][index_q],
                l,
                y[temp_idx+index_q*(pv->l_max_ncdm[n_ncdm]+1)+l],
                dy[temp_idx+index_q*(pv->l_max_ncdm[n_ncdm]+1)+l],
                y[temp_idx+index_q*(pv->l_max_ncdm[n_ncdm]+1)+l] / pvecback[pba->index_bg_taurel_si_ncdm1] );
              }
            }
          }*/
        }
          ...
          if( (ppw->approx[ppw->index_ap_ncdmfa] == (int)ncdmfa_on) ) {
            ...
            /*if(ppw->approx[ppw->index_ap_ncdmtca_ncdm1+n_ncdm] == (int)ncdmtca_on){
              if(tau == pba->tau_table[0]){ 
                printf("Detected ncdm_tca on species %d \n",n_ncdm);
              }
            }*/
