...
#define _QUADRATURE_MAX_ 500 /**< maximum allowed number of abssices in quadrature integral estimation */

#define _QUADRATURE_MAX_BG_ 1000 /**< maximum allowed number of abssices in quadrature integral estimation */
...
/**
 * Different estimation methods for high momenta quatities in ncdm hierarchies
 */

enum ncdm_ic_estimation_method {zeros_method,tridiag_method};

/**
 * Different ways of closing the ncdm hierarchies at l_max
 */

enum ncdm_lmax_method {mb_method,y_method};

/**
 * Method to use in the case of a non relativistic self interaction decoupling
 */

enum ncdm_NR_SI_decoupling_method {NR_SI_ignore,NR_SI_extrapolateDF_microK,NR_SI_extrapolateDF_iEntropy};
...
/* @endcond */
  ...
  /**
   * Parameter controlling which method is used to estimate high momenta quantities in ncdm hierarchies
   */ 

  int ncdm_ic_estimation_method;

  /**
   * Parameter controlling which method is used to close ncdm hierarchies at l_max
   */

  int ncdm_lmax_method;
  ...
  /**
   * when to switch on si-ncdm (massive neutrinos / non-cold
   * relics) tight coupling approximation
   */
  double ncdm_si_tca_trigger;

  /* Parameter controlling how 
  strongly coupled must ncdm SI be at the initial time */
  double ncdm_si_initial_tau_trigger;
